...
description: Builds a polished, responsive personal portfolio website in plain HTML/CSS/JS. Use when the user asks to "build a portfolio", "make a portfolio site", "create my personal website", or similar.
...
2. **Confirm the target directory.** Ask where to write the site (default:
   ...
   files without checking first — Glob the target and warn if it's non-empty.
...
4. **Verify before declaring done.**
   - Open `index.html` and confirm every section the user asked for is present
     and populated (no leftover placeholder unless intentionally a TODO).
   ...
   - Offer to start a local preview server so they can see it:
